...
import { CustomerCard, CustomerFilterTabs, SearchInput, Topic } from "@/components";
...
const Customers = () => {
    ...
    const { data: customersData, isLoading } = useGetCustomers(`search=${debouncedQuery}&filter=${selected.toLowerCase()}`);
    return <div>
        ...
        {!isLoading ? <div className="mt-2 flex flex-col gap-2 overflow-y-scroll h-[calc(100vh-308px)] sm:h-[calc(100vh-240px)]">
            {customersData?.data?.data?.map((e: {
                ...
                return <div key={`${e.customerName}-${index}`} onClick={()=>router.push(`/customers/${e._id}`)}>
                    <CustomerCard
                        customerName={e.name}
